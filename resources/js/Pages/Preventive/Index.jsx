...
export default function Index({ auth, records, filters }) {
    ...
    const handleDismiss = (id) => {
        if (confirm('¿Estás seguro de descartar este registro del monitor? No se eliminará del historial de la mascota, pero ya no aparecerá aquí.')) {
            router.post(route('preventive-records.dismiss', id), { type: activeMonitor }, {
                preserveScroll: true,
                onSuccess: () => {
                    // Optional: add a success notification if the system supports it
                }
            });
        }
    };
                        ...
                        <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col md:flex-row gap-4 items-end">
                            <div className="flex-1 w-full">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-purple/30 focus:border-brand-purple transition shadow-inner"
                                />
                            </div>
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className={`text-white px-5 py-2 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg transition active:scale-95 ${activeMonitor === 'health' ? 'bg-indigo-600 shadow-indigo-100' : 'bg-brand-primary shadow-brand-primary/20'}`}
                                >
                                    Filtrar
                                </button>
                                {activeMonitor === 'health' && (
                                    <a
                                        ...
                                        target="_blank"
                                        className="bg-emerald-500 text-white px-5 py-2 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-emerald-100 hover:bg-emerald-600 transition active:scale-95 flex items-center gap-2"
                                    >
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-slate-50/50 dark:bg-gray-900/40 text-left">
                                        <th className="px-4 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest">Paciente / Dueño</th>
                                        <th className="px-4 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest">{activeMonitor === 'health' ? 'Tratamiento' : 'Último Servicio'}</th>
                                        <th className="px-4 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest">{activeMonitor === 'health' ? 'Última Aplicación' : 'Fecha Última Visita'}</th>
                                        <th className="px-4 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Próxima Visita / Refuerzo</th>
                                        ...
                                        records.data.map(item => {
                                            ...
                                            return (
                                                <tr 
                                                    ...
                                                    onClick={() => {
                                                        if (activeMonitor === 'grooming') {
                                                            router.visit(route('grooming-orders.show', item.id));
                                                        } else {
                                                            router.visit(route('pets.show', item.pet.id));
                                                        }
                                                    }}
                                                >
                                                    ...
                                                    <td className="px-4 py-2.5 text-xs text-gray-500 dark:text-gray-400 font-bold uppercase transition">
                                                    ...
                                                    <td className="px-4 py-2.5 text-center" onClick={(e) => e.stopPropagation()}>
                                                        <div className="flex justify-center gap-2">
                                                            ...
                                                            {/* Botón Agendar — solo para Estética */}
                                                            {activeMonitor === 'grooming' && (
                                                                <a
                                                                    href={route('appointments.index') + `?prefill_pet_id=${item.pet?.id}&prefill_type=grooming`}
                                                                    className="p-2 bg-brand-primary text-white rounded-xl hover:bg-brand-primary/80 transition shadow-lg flex items-center gap-1 px-3"
                                                                    title="Agendar cita de Estética para esta mascota"
                                                                >
                                                                    ...
                                                                    <span className="text-[9px] font-black uppercase tracking-widest hidden sm:inline">Agendar</span>
                                                                </a>
                                                            )}
                                                            <button 
                                                                onClick={() => handleDismiss(item.id)}
                                                                className="p-2 text-slate-400 hover:text-red-500 transition-all opacity-0 group-hover:opacity-40 hover:!opacity-100"
                                                                title={activeMonitor === 'grooming' ? 'No interesado — Descartar del monitor' : 'Descartar / Ocultar del Monitor'}
                                                            >
