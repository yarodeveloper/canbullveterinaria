...
export default function Edit({ auth, quote, templates, products, settings }) {
    const [selectedTemplate, setSelectedTemplate] = useState('');
    const [selectedPet, setSelectedPet] = useState(quote.pet || null);
    const [petWeight, setPetWeight] = useState(quote.weight_at_time || quote.pet?.weight || 0);
    const [isGuest, setIsGuest] = useState(!quote.pet_id);

    const { data, setData, put, processing, errors } = useForm({
        is_guest: !quote.pet_id,
        pet_id: quote.pet_id || '',
        client_id: quote.client_id || '',
        guest_client_name: quote.guest_client_name || '',
        guest_pet_name: quote.guest_pet_name || '',
        guest_species: quote.guest_species || 'Canino',
    ...
    const toggleGuest = (val) => {
        ...
        if (val) {
            ...
            if (quote.pet) {
                setSelectedPet(quote.pet);
                setData(prev => ({
                    ...prev,
                    pet_id: String(quote.pet_id),
                    client_id: String(quote.client_id)
                }));
                setPetWeight(quote.weight_at_time || quote.pet.weight || 0);
            }
        }
    };
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex items-center gap-3">
                    <Link href={route('quotes.show', quote.id)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition">
            ...
            <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
                <form onSubmit={submit}>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 p-8 mb-6">
                        <div className="flex justify-between items-start mb-6 pb-6 border-b dark:border-slate-700">
                            <div>
                                ...
                                <p className="text-xs text-slate-400 font-bold mt-1">Modifica los detalles generales de la cotización.</p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            {!isGuest && (
                                <>
                                    {selectedPet ? (
                                        <div className="md:col-span-3 flex items-center justify-between bg-brand-primary/5 dark:bg-brand-primary/10 p-4 rounded-xl border border-brand-primary/20">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="font-bold text-slate-900 dark:text-white text-lg">{selectedPet.name}</p>
                                                    <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Dueño: {selectedPet.owner?.name}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-4">
                                                <div className="text-right">
                                                    <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Peso Registrado</p>
                                                    <p className="font-black text-brand-primary">{selectedPet.weight} kg</p>
                                                </div>
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        setSelectedPet(null);
                                                        setData(prev => ({ ...prev, pet_id: '', client_id: '' }));
                                                    }}
                                                    className="px-3 py-1 bg-red-100 text-red-600 hover:bg-red-500 hover:text-white rounded-lg text-[10px] font-black uppercase transition"
                                                >
                                                    Cambiar
                                                </button>
                                            </div>
                                    ...
                                    {!selectedPet && (
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2">Peso Estimado (kg)</label>
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2">Estado</label>
                            ...
                            <div>
                                ...
                                <p className="text-[9px] text-slate-400 mt-1 ml-1">Agregará los ítems al detalle actual.</p>
                            </div>
                    ...
                    <div className="flex justify-end gap-4">
                        <Link href={route('quotes.show', quote.id)} className="px-8 py-3.5 border border-slate-200 dark:border-slate-700 text-slate-500 rounded-xl font-black uppercase text-xs hover:bg-slate-50 transition">
                            Cancelar
                        </Link>
                        <button
                            type="submit"
