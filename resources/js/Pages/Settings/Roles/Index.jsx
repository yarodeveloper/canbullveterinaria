        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex justify-between items-center">
                    <h2 className="text-2xl font-black text-gray-800 dark:text-gray-200">Roles y Permisos</h2>
                    <Link
                        href={route('roles.create')}
                        className="px-6 py-2 bg-brand-primary text-white rounded-xl text-xs font-black uppercase hover:bg-brand-primary/90 transition shadow-lg shadow-brand-primary/20"
                    >
                        Crear Rol
                    </Link>
                </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-gray-900/50">
                                        <th className="p-6 text-xs font-black uppercase text-gray-400 tracking-widest">Nombre del Rol</th>
                                        <th className="p-6 text-xs font-black uppercase text-gray-400 tracking-widest">Permisos Asignados</th>
                                        <th className="p-6 text-xs font-black uppercase text-gray-400 tracking-widest text-right">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                    {roles.map((role) => (
                                        <tr key={role.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition">
                                            <td className="p-6">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-brand-primary font-medium">#{role.id}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-6">
                                                <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-tighter bg-indigo-100 text-indigo-700">
                                                    {role.name === 'admin' ? 'Todos los permisos' : `${role.permissions_count} permisos`}
                                                </span>
                                            </td>
                                            <td className="p-6 text-right">
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    {/* Hide delete button for essential roles */}
